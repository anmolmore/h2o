import h2o
import pandas as pd
#Complete run to make 5 base models, 1 ensemble around 20 CV models, takes around 3-5 minutes on single cluster, 1.7 GB, 4 cores
...
print("\n\n ############# Compare models for RMSE on validation set, XGBoost and Deep learning gives desired result ################### ")
all_models = [glm,rFm,mGBM, xgb, ensemble,DL_final_model]
names = ["Generalized Linear Model","Random Forest","GBM","XGBoost","Ensemble","Deep Learning"]
print(pd.Series(map(lambda x:x.model_performance(valid)['RMSE'],all_models),names))
